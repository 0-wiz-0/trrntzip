  ...
  while (pfile_in_zip_read_info->stream.avail_out > 0) {
    ...
    if ((pfile_in_zip_read_info->compression_method == 0) ||
      ...
      if (err == BZ_STREAM_END) {
        // FIX STATMAT
        // It's sometimes possible (under rare circumstances) that there is an
        // error with the very last byte of the compressed file (i.e. it is not
        // read due to file corruption). zlib returns that we have met the end
        // of the compressed file, so the code below just returns this fact (no
        // error). Simply added a new check to handle this situation.
        if (iRead == 0 && pfile_in_zip_read_info->crc32 !=
